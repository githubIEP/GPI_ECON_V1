...
# Text string to use inside function call to only select certain countries
COUNTRY_CLAUSE = paste("WHERE geocode IN ('", paste(CONFLICT_CHECK, collapse = "','"), "')", sep = "")

### --- Load the data

# Load ACLED, selecteding only countries in the CONFLICT_CHECK list
acled.raw <- iepg_acled(clause = COUNTRY_CLAUSE)
match_acled.df <- acled.raw %>%
  filter(year >= CONFLICT_CHECK_START) %>%
...
match_tt.df <- f_GetTT(CUTOFF_DATA,CUTOFF_PULL) %>%
  select(event_date = start_date, geocode, country, year, 
         tt_latitude=latitude, tt_longitude = longitude,
         tt_summary = summary, tt_event_type = event_type, tt_total_deaths = total_deaths, 
         tt_total_injured = total_injured, tt_perpetrators = perpetrator_name, 
